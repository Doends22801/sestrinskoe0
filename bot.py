...
class UserProgress:
    def __init__(self):
        ...
        self.mistakes_practice_mode = False  # Режим отработки ошибок
        self.mistakes_to_practice = []  # Ошибки для отработки
    ...
    def start_mistakes_practice(self):
        ...
        if self.mistakes:
            ...
            # Создаем вопросы для отработки из ошибок
            ...
            # Перемешиваем вопросы с ошибками
            random.shuffle(self.mistakes_to_practice)
            self.current_question = 0
            self.score = 0
            self.current_attempts = 0
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    🔄 В режиме отработки ошибок вы будете отвечать только на вопросы, в которых ошиблись, до тех пор пока не ответите правильно!
    """
...
async def send_question(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    if progress.mistakes_practice_mode:
        question_text = f"📝 {progress_text}{attempts_text}\nВопрос: {question_data['question']}"
    else:
        question_text = f"{progress_text}{attempts_text}\nВопрос: {question_data['question']}"
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not progress.is_test_complete():
        ...
        if progress.mistakes_practice_mode:
            keyboard.append(
                [InlineKeyboardButton("🏁 Завершить отработку ошибок", callback_data="finish_mistakes_practice")])
        else:
...
async def finish_mistakes_practice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    progress = user_data[user_id]
    corrected_mistakes = len(progress.mistakes_to_practice)

    if corrected_mistakes == 0:
        result_text = "🎉 Поздравляем! Вы исправили все ошибки! 🏆"
    else:
        result_text = f"📊 Отработка завершена!\nОсталось ошибок: {corrected_mistakes}"

    keyboard = [
        [InlineKeyboardButton("📝 Посмотреть оставшиеся ошибки", callback_data="view_mistakes")],
        [InlineKeyboardButton("🔄 Начать новый тест", callback_data="restart_test")],
...
async def finish_test(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, early_exit=False):
    ...
    if progress.mistakes:
        ...
        result_text += "Используйте /my_mistakes чтобы отработать ошибки"
    else:
